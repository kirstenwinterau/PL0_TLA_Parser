...
Init == /\ stack \in FiniteSeq(T, N) /\ head \in Ref \union {null} /\ mem \in [Ref ->(T \X (Ref \union {null})) \union {undef}] /\ ABS /\ ss = null /\ pc = 15 /\ v \in T /\ n \in Ref /\ INV /\ in \in T /\ lv = 0  /\ out = 0  /\ ssn = 0  /\ STATUS
...
push15 ==  pc = 15 /\ pc' = 16 /\ n' \in Ref /\ (\A r\in Ref : mem[r][2] /= n' /\ (mem[r] /= undef => r /= n')) /\ mem'= [mem EXCEPT ![n'] = <<0,null>>] /\ UNCHANGED<<ss,stack,nq,in,lvq,lv,inq,out,ssn,head,ssq,ssnq,v,outq,vq,pcq>> 

Spec ==  Init /\ [][push15]_<<ss, stack, nq, in, lvq, lv, inq, n, out, ssn, head, ssq, pc, mem, ssnq, v, outq, vq, pcq>> 
