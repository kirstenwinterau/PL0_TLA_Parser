...
public class SymbolTable {
    ...
    public SymbolTable() {
        ...
        SymEntry.ProcedureEntry predefined = 
                ...
                Position.NO_POSITION, null, null, null );
...
//          System.out.println( "Symtab resolving " + entry.ident );
    ...
    public SymEntry.VarEntry addVariable(String name, Position posn, 
            ReferenceType type, boolean isGlobal, boolean isAbstract ){
        SymEntry.VarEntry entry =
                new SymEntry.VarEntry(name, posn, currentScope, type, isGlobal, isAbstract );
        return (SymEntry.VarEntry) currentScope.addEntry(entry);
    }
    public SymEntry.VarEntry addVariable(String name, Position posn, 
            ReferenceType type ){
        SymEntry.VarEntry entry =
                new SymEntry.VarEntry(name, posn, currentScope, type, false, false );
        return (SymEntry.VarEntry) currentScope.addEntry(entry);
    }
     ...
     *         already exists in the current scope, in which case return null.
     */
    public SymEntry.ProcedureEntry addProcedure(String name, Position posn, List<Type> returnTypes, ExpNode args) {
        SymEntry.ProcedureEntry entry =
                new SymEntry.ProcedureEntry(name, posn, currentScope, returnTypes, args );
    ...
    public SymEntry.ProcedureEntry addProcedure(String name, Position posn,
            Type.ProcedureType type, List<Type> returnTypes ) {
        SymEntry.ProcedureEntry entry =
                new SymEntry.ProcedureEntry(name, posn, currentScope, type, returnTypes, null );
        return (SymEntry.ProcedureEntry) currentScope.addEntry(entry);
    }
